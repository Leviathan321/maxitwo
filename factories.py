...
from envs.beamng.config import MIN_SPEED_BEAMNG, MAX_SPEED_BEAMNG
...
def make_env(
    ...
    if cyclegan_experiment_name is not None:
        ...
        logger.info("Loaded cyclegan model")
        logger.info(cyclegan_model.device)
...
def make_agent(
    ...
    if agent_type == "autopilot":
        return AutopilotAgent(
            env=env, env_name=env_name, max_speed=max_speed, min_speed=min_speed
        )
